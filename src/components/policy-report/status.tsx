...
type StatusTypes = 'pass' | 'fail' | 'warn' | 'error' | 'skip' | 'remediationsAvailable';

const useGetStatusColor = (
  status:
    | TMayBe<TPolicyReportResultData_Status_ClusterService | TPolicyReportResultData_Status_PoliciesService>
    | string
    | StatusTypes
): string | undefined => {
  ...
  const colorMapping: Record<
    TPolicyReportResultData_Status_ClusterService | TPolicyReportResultData_Status_PoliciesService | string,
...
const StatusTag: React.FC<{
  status?: TMayBe<TPolicyReportResultData_Status_ClusterService | TPolicyReportResultData_Status_PoliciesService>;
  ...
  return (
    <Tag className={className} style={style} color={color}>
      {capitalize(status ?? emptyFallbackValue)}
    </Tag>
  );
};
...
const CountTag: React.FC<{
  ...
  const finalStatus = countBasedColor ? (!count ? 'skip' : status) : count === undefined ? 'skip' : status;
